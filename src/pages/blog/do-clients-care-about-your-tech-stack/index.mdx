...
import conferenceRoom from './conference_room.webp';
...
<Image
  ...
  src={conferenceRoom}
  alt="A freelance developer pitching React to an uninterested and apathetic conference room"
/>
...
In this case, the above (fictitious) tweet rings true.
...
There is always a hope in agency life or freelance life that, assuming the working relationship is good and mutually beneficial, that a client will come back
with more work. But for any number of reasons, this is not always what actually happens.

To get personal for a moment, my favorite technology that I have ever worked with is Elixir. I enjoy the language, the philosophies, the tooling, and the community.
...
that give us a reasonable expectation that they will be around for the foreseeable future. This is not always the case with newer technologies.
...
operating system? Does it have specific requirements that will tether the client to a specific cloud provider? For example, you don't _have_ to deploy NextJS
...
What sort of technical skills are necessary to deploy changes? Can a non-technical user simply login to a CMS and make changes to content? Or will they need to
...
websites from Expedia to Kayak and etc have a very lengthy checkout process relative to say purchasing a product on Amazon. You don't want to build a system
that is significantly slower than it could be, but it's not a situation where milliseconds matter.
...
every technology choice we make is an exercise in trade-offs and it's critical that we make our decisions with the client's best interest in mind and always
try to balance the trade-offs in their favor. Nothing will ever be perfect, but we should do our best.

Clients do care deeply about the cons and negative trade offs of our technology choices, even if they can't explain the technical specifics of why a certain outcome
occurred. As freelancers and consultants, they trust us to make the right decisions on their behalf and it's critical we do our best to not betray that trust.
